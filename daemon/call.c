		...
		const struct stream_params *sp)
{
	...
	if (PS_ISSET(ps, FILLED)) {
		...
		if (!memcmp(&ps->advertised_endpoint, &ep, sizeof(ep)))
			return;

		/* if this stream is ICE enabled, we use the ICE fragment to determine
		 * whether this is a change or not */
		if (media->ice_agent && MEDIA_ISSET(media, ICE) && sp) {
			if (!ice_ufrag_cmp(media->ice_agent, sp)) {
